...
function clearErrors(
  ...
  if (prevErrors.length !== 0) {
    field.errors = [];
    events.push({ type: 'errorsCleared', target: field, relatedTarget: field });
  }
...
function getInvalidFields(field: Field<any, ErrorsMixin>, batch: Field<any, ErrorsMixin>[]): Field<any, ErrorsMixin>[] {
  if (field.isInvalid) {
    batch.push(field);
  }
