 ...
 */
export type InferValue<T> = 'value' extends keyof T ? T['value'] : any;
...
export interface FieldEvent<Mixin = any> {
  ...
  /**
   * The field that caused this event to be published onto the {@link target target field}.
   *
   * For example, if a child field value is changed and causes the change event to be published for the parent field
   * as well, then the origin field is the child field for both change events.
   */
  relatedTarget: Field<any, Mixin>;
}
...
function setValue(field: Field, value: unknown, isTransient: boolean): void {
  ...
  for (const event of propagateValue(root, field, value, [])) {
...
function propagateValue(target: Field, relatedTarget: Field, value: unknown, events: FieldEvent[]): FieldEvent[] {
  events.push({ type: 'change', target, relatedTarget });
